		...
		communityAcceptInviteV4: ev.createBufferedFunction(
			async (key: string | WAMessageKey, inviteMessage: proto.Message.IGroupInviteMessage) => {
				...
				await upsertMessage(
					{
						key: {
							remoteJid: inviteMessage.groupJid ?? null,
							id: generateMessageIDV2(sock.user?.id),
							fromMe: false,
							participant: key.remoteJid ?? null
						},
...
export const extractCommunityMetadata = (result: BinaryNode) => {
	...
	const metadata: any = {
		...
		creation: Number(community.attrs.creation || 0),
			owner: community.attrs.creator ? jidNormalizedUser(community.attrs.creator) : '',
		desc,
		descId,
		...
		ephemeralDuration: eph ? +eph : undefined,
			addressingMode: (getBinaryNodeChildString(community, 'addressing_mode') || 'pn') as GroupMetadata['addressingMode']
	}
