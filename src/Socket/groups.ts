			...
			await groupQuery(jid, 'set', [
				{
					...
					content: description ? [{ tag: 'body', attrs: {}, content: Buffer.from(description, 'utf-8') }] : []
				}
			])
		},
		...
		groupAcceptInviteV4: ev.createBufferedFunction(
			async (key: string | WAMessageKey, inviteMessage: proto.Message.IGroupInviteMessage) => {
				...
				if (key.id) {
					...
					if (key.id) {
						// create a minimal WAMessage to mark the invite as expired and emit an update
						const msg: any = {
							key: { remoteJid: inviteMessage.groupJid ?? null, id: key.id, fromMe: false, participant: key.remoteJid ?? null },
							message: { groupInviteMessage: inviteMessage },
							messageTimestamp: unixTimestampSeconds()
						}
						try {
							ev.emit('messages.update', { update: msg, key: msg.key } as any)
						} catch {
							// ignore emit errors
						}
					}
	...
	const metadata: any = {
		...
		memberAddMode,
			participants: getBinaryNodeChildren(group, 'participant').map(({ attrs }) => {
				return {
					id: attrs.jid!,
					phoneNumber: isLidUser(attrs.jid) && isPnUser(attrs.phone_number) ? attrs.phone_number : '',
					lid: isPnUser(attrs.jid) && isLidUser(attrs.lid) ? attrs.lid : '',
					admin: (typeof attrs.type === 'undefined' ? null : attrs.type) as GroupParticipant['admin']
				} as any
			}) as any,
