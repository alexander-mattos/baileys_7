...
import { trimUndefined } from '../Utils/generics'
...
export const makeMessagesSocket = (config: SocketConfig) => {
	...
	const relayMessage = async (
		...
		if (participant) {
			...
			devices.push({
				user,
				device: (typeof device === 'number' ? device : 0),
				jid: participant.jid
			})
		}
	...
	return {
		...
		updateMediaMessage: async (message: WAMessage) => {
					...
					if (result) {
						if (result.error) {
							...
							try {
								...
								content.directPath = media.directPath ?? null
			...
			ev.emit('messages.update', [{ key: message.key, update: { message: message.message ?? null } }])
		...
		sendMessage: async (jid: string, content: AnyMessageContent, options: MiscMessageGenerationOptions = {}) => {
			...
			if (
					...
					userJid,
						getUrlInfo: text =>
							getUrlInfo(text, {
								...
								fetchOpts: {
									timeout: 3_000,
									...trimUndefined(httpRequestOptions || {})
								},
								logger,
								...(generateHighQualityLinkPreview ? { uploadImage: waUploadToServer } : {})
							}),
					//TODO: CACHE
					getProfilePicUrl: sock.profilePictureUrl,
					getCallLink: sock.createCallLink,
					upload: waUploadToServer,
					...(config.mediaCache ? { mediaCache: config.mediaCache } : {}),
					...(config.options ? { options: trimUndefined(config.options || {}) } : {}),
				...
				await relayMessage(jid, fullMsg.message!, {
					...
					...(Object.keys(additionalAttributes).length ? { additionalAttributes } : {}),
