...
export const makeMessagesRecvSocket = (config: SocketConfig) => {
	...
	const fetchMessageHistory = async (
		...
		const pdoMessage: proto.Message.IPeerDataOperationRequestMessage = {
			historySyncOnDemandRequest: {
				chatJid: oldestMsgKey.remoteJid ?? null,
				oldestMsgFromMe: oldestMsgKey.fromMe ?? null,
				oldestMsgId: oldestMsgKey.id ?? null,
	...
	const handleNewsletterNotification = async (node: BinaryNode) => {
		...
		switch (child.tag) {
			case 'reaction':
				...
				ev.emit('newsletter.reaction', reactionUpdate as any)
				break
	...
	const sendMessageAck = async ({ tag, attrs, content }: BinaryNode, errorCode?: number) => {
		...
		if (
			!!attrs.type &&
			(tag !== 'message' || getBinaryNodeChild({ tag, attrs, content: content ?? [] }, 'unavailable') || errorCode !== 0)
		) {
			stanza.attrs.type = attrs.type
		}

		if (tag === 'message' && getBinaryNodeChild({ tag, attrs, content: content ?? [] }, 'unavailable')) {
	...
	const handleGroupNotification = (participant: string, child: BinaryNode, msg: Partial<WAMessage>) => {
		...
		switch (child?.tag) {
			case 'create':
				...
				msg.key = { participant: metadata.owner ?? null }

				ev.emit('chats.upsert', [
					{
						id: metadata.id,
						name: metadata.subject,
						conversationTimestamp: metadata.creation ?? null
					}
				])
			...
			case 'description':
				...
				msg.messageStubParameters = description ? [description] : []
				break
			case 'announcement':
		...
		switch (nodeType) {
			...
			case 'picture':
				...
				if (isJidGroup(from)) {
					...
					result.participant = node?.attrs.author ?? null
					result.key = {
						...(result.key || {}),
						participant: setPicture?.attrs.author ?? null
					}
				}
	...
	const handleReceipt = async (node: BinaryNode) => {
		...
		const key: proto.IMessageKey = {
			...
			id: '',
			fromMe,
			participant: attrs.participant ?? null
		}
			...
			await Promise.all([
				processingMutex.mutex(async () => {
					...
					if (attrs.type === 'retry') {
						...
						key.participant = key.participant ?? attrs.from ?? null
				...
				processingMutex.mutex(async () => {
					...
					if (msg) {
						...
						msg.key = {
							remoteJid: remoteJid ?? null,
							fromMe,
							participant: node.attrs.participant ?? null,
							id: node.attrs.id ?? null,
							...(msg.key || {})
						}
						msg.participant = msg.participant ?? node.attrs.participant ?? null
	...
	const handleCall = async (node: BinaryNode) => {
		...
		if (status === 'offer') {
			...
			if (infoChild.attrs['group-jid']) call.groupJid = infoChild.attrs['group-jid']
	...
	const handleBadAck = async ({ attrs }: BinaryNode) => {
		const key: WAMessageKey = { remoteJid: attrs.from ?? null, fromMe: true, id: attrs.id ?? null }
