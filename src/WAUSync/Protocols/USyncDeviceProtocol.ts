		...
		if (node.tag === 'devices') {
			...
			if (keyIndexNode?.tag === 'key-index-list') {
				keyIndex = {
					...
					// only set signedKeyIndex if content is a Uint8Array
					...(keyIndexNode?.content ? { signedKeyIndex: keyIndexNode.content as Uint8Array } : {}),
					...(keyIndexNode.attrs['expected_ts'] ? { expectedTimestamp: +keyIndexNode.attrs['expected_ts'] } : {})
				}
			}
		}
