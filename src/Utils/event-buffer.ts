	...
	switch (event) {
		case 'messaging-history.set':
			for (const chat of eventData.chats as Chat[]) {
				...
				if (existingChat) {
					existingChat.endOfHistoryTransferType = chat.endOfHistoryTransferType ?? null
				}
			...
			data.historySets.syncType = eventData.syncType ?? null
			data.historySets.progress = eventData.progress ?? null
			data.historySets.peerDataRequestSessionId = eventData.peerDataRequestSessionId ?? null
		...
		case 'messages.upsert':
			...
			for (const message of messages) {
				...
				if (existing) {
					message.messageTimestamp = existing.messageTimestamp ?? null
				}
...
function consolidateEvents(data: BufferedEventData) {
	...
	if (!data.historySets.empty) {
		map['messaging-history.set'] = {
			...
			syncType: data.historySets.syncType ?? null,
			progress: data.historySets.progress ?? null,
			isLatest: data.historySets.isLatest,
			peerDataRequestSessionId: data.historySets.peerDataRequestSessionId ?? null
		}
	}
