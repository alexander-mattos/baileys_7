...
export function decodeMessageNode(stanza: BinaryNode, meId: string, meLid: string) {
	...
	const key: WAMessageKey = {
		remoteJid: chatId ?? null,
		fromMe,
		id: msgId ?? null,
		participant: participant ?? null,
		addressingMode: addressingContext.addressingMode,
		...(msgType === 'newsletter' && stanza.attrs.server_id ? { server_id: stanza.attrs.server_id } : {}),
		...(!isJidGroup(chatId) && addressingContext.senderAlt ? { remoteJidAlt: addressingContext.senderAlt! } : {}),
		...(isJidGroup(chatId) && addressingContext.senderAlt ? { participantAlt: addressingContext.senderAlt! } : {})
	}
	...
	const fullMessage: WAMessage = {
		...
		broadcast: isJidBroadcast(from) ?? null
	}
			...
			if (Array.isArray(stanza.content)) {
				for (const { tag, attrs, content } of stanza.content) {
					if (tag === 'verified_name' && content instanceof Uint8Array) {
						...
						fullMessage.verifiedBizName = details.verifiedName ?? null
					}
