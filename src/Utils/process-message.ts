...
export const cleanMessage = (message: WAMessage, meId: string) => {
	...
	function normaliseKey(msgKey: WAMessageKey) {
		...
		if (!message.key.fromMe) {
			...
			msgKey.remoteJid = message.key.remoteJid ?? null
			// set participant of the message
			// use nullish coalescing to avoid mixing || and ?? which breaks some JS transforms
			msgKey.participant = msgKey.participant ?? message.key.participant ?? null
		}
	}
}
...
export const getChatId = ({ remoteJid, participant, fromMe }: WAMessageKey) => {
	// for broadcast messages, the chat id is the participant (the sender),
	// otherwise it's the remoteJid
...
const processMessage = async (
	...
	if (isRealMsg) {
		chat.messages = [{ message }]
				chat.conversationTimestamp = toNumber(message.messageTimestamp) ?? null
	...
	if (protocolMsg) {
			...
			case proto.Message.ProtocolMessage.Type.HISTORY_SYNC_NOTIFICATION:
				...
				if (process) {
					if (histNotification.syncType !== proto.HistorySync.HistorySyncType.ON_DEMAND) {
						// ensure existing processedHistoryMessages entries do not contain undefined messageTimestamp
						const processedExisting = (creds.processedHistoryMessages || []).map(m => ({ key: m.key, messageTimestamp: m.messageTimestamp ?? null }))
						ev.emit('creds.update', {
							processedHistoryMessages: [
								...processedExisting,
								{ key: message.key, messageTimestamp: message.messageTimestamp ?? null }
							]
						})
					}
					...
					ev.emit('messaging-history.set', {
						...
						peerDataRequestSessionId: histNotification.peerDataRequestSessionId ?? null
					})
				}
					...
					ev.emit('messages.update', [
						{
							...
							key: { ...message.key, id: protocolMsg.key?.id ?? null },
							update: {
								message: {
									editedMessage: {
										message: protocolMsg.editedMessage ?? null
									}
								},
		...
		const emitParticipantsUpdate = (action: ParticipantAction) =>
			ev.emit('group-participants.update', { id: jid, author: message.participant!, participants, action })
				const emitGroupUpdate = (update: Partial<GroupMetadata>) => {
					ev.emit('groups.update', [{ id: jid, ...update, ...(message.participant ? { author: message.participant } : {}) }])
				}

				const emitGroupRequestJoin = (participant: string, action: RequestJoinAction, method: RequestJoinMethod) => {
					ev.emit('group.join-request', { id: jid, participant, action, method: method!, author: message.participant ?? '' })
				}
		...
		switch (message.messageStubType) {
			...
			case WAMessageStubType.GROUP_CHANGE_SUBJECT:
				...
				chat.name = name ?? null
				if (typeof name !== 'undefined') emitGroupUpdate({ subject: name })
				break
			case WAMessageStubType.GROUP_CHANGE_DESCRIPTION:
				const description = message.messageStubParameters?.[0]
				chat.description = description ?? null
				if (typeof description !== 'undefined') emitGroupUpdate({ desc: description })
				break
			case WAMessageStubType.GROUP_CHANGE_INVITE_LINK:
				const code = message.messageStubParameters?.[0]
				if (typeof code !== 'undefined') emitGroupUpdate({ inviteCode: code })
				break
