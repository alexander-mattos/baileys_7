...
const assertColor = async (color: any) => {
    ...
    // ensure we always return the computed color
    return assertedColor
}
...
export const prepareWAMessageMedia = async (
	...
	if (isNewsletter) {
		...
		const uploadOpts: any = { fileEncSha256B64: fileSha256B64, mediaType: mediaType }
		if (typeof options.mediaUploadTimeoutMs === 'number') uploadOpts.timeoutMs = options.mediaUploadTimeoutMs
		const { mediaUrl, directPath } = await options.upload(filePath, uploadOpts)
		...
		if (uploadData.ptv) {
			obj.ptvMessage = obj.videoMessage ?? null
	...
	// build encryptedStream options only with defined values to avoid passing undefined to required fields
	const encOpts: any = { saveOriginalFileIfRequired: requiresOriginalForSomeProcessing }
	if (logger) encOpts.logger = logger
	if (options.options) encOpts.opts = options.options
	...
	const [{ mediaUrl, directPath }] = await Promise.all([
		(async () => {
				const uploadOpts2: any = { fileEncSha256B64, mediaType }
				if (typeof options.mediaUploadTimeoutMs === 'number') uploadOpts2.timeoutMs = options.mediaUploadTimeoutMs
				const result = await options.upload(encFilePath, uploadOpts2)
		...
		(async () => {
			try {
				if (requiresThumbnailComputation) {
					...
					if (typeof thumbnail === 'string') uploadData.jpegThumbnail = thumbnail
				...
				if (requiresDurationComputation) {
					const dur = await getAudioDuration(originalFilePath!)
					if (typeof dur === 'number') uploadData.seconds = dur
					logger?.debug('computed audio duration')
				}

				if (requiresWaveformProcessing) {
					const wf = await getAudioWaveform(originalFilePath!, logger)
					if (wf) uploadData.waveform = wf
					logger?.debug('processed waveform')
				}

				if (requiresAudioBackground) {
					const bg = await assertColor(options.backgroundColor)
					if (typeof bg === 'number') uploadData.backgroundArgb = bg
...
export const generateForwardMessageContent = (message: WAMessage, forceForward?: boolean) => {
	...
	if (key === 'conversation') {
		content.extendedTextMessage = { text: (content[key] ?? null) as any }
...
export const generateWAMessageContent = async (
	...
	if ('text' in message) {
		...
		if (urlInfo) {
			extContent.matchedText = urlInfo['matched-text'] ?? null
		// ensure values conform to proto fields which often expect string | null
			extContent.jpegThumbnail = urlInfo.jpegThumbnail ?? null
			extContent.description = urlInfo.description ?? null
			extContent.title = urlInfo.title ?? null
			extContent.previewType = 0

			const img = urlInfo.highQualityThumbnail
				if (img) {
					// many proto fields are typed as `string | null` or `Uint8Array | null`
					extContent.thumbnailDirectPath = img.directPath ?? null
					extContent.mediaKey = img.mediaKey ?? null
					extContent.mediaKeyTimestamp = img.mediaKeyTimestamp ?? null
					extContent.thumbnailWidth = img.width ?? null
					extContent.thumbnailHeight = img.height ?? null
					extContent.thumbnailSha256 = img.fileSha256 ?? null
					extContent.thumbnailEncSha256 = img.fileEncSha256 ?? null
				}
		}
		...
		m.ptvMessage = videoMessage ?? null
		...
		m.eventMessage.description = message.event.description ?? null
		m.eventMessage.startTime = startTime
		m.eventMessage.endTime = message.event.endDate ? message.event.endDate.getTime() / 1000 : null
		m.eventMessage.isCanceled = message.event.isCancelled ?? false
		m.eventMessage.extraGuestsAllowed = message.event.extraGuestsAllowed ?? null
		m.eventMessage.isScheduleCall = message.event.isScheduleCall ?? false
		m.eventMessage.location = message.event.location ?? null
...
export const generateWAMessageFromContent = (
	...
	contextInfo.stanzaId = quoted.key.id ?? null
		contextInfo.quotedMessage = quotedMsg

		// if a participant is quoted, then it must be a group
		// hence, remoteJid of group must also be entered
		if (jid !== quoted.key.remoteJid) {
			contextInfo.remoteJid = quoted.key.remoteJid ?? null
		}

		if (contextInfo && innerMessage[key]) {
			const k: any = innerMessage[key]
			if (contextInfo.remoteJid) contextInfo.remoteJid = contextInfo.remoteJid ?? null
			k.contextInfo = contextInfo
		}
	}
	...
	async function downloadMsg() {
		...
		if ('thumbnailDirectPath' in media && !('url' in media)) {
			download = {
				...
				mediaKey: (media.mediaKey as any) ?? null
			}
