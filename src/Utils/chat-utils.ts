...
export const extractSyncdPatches = async (result: BinaryNode, options: RequestInit) => {
	...
	await Promise.all(
		collectionNodes.map(async collectionNode => {
			...
			final[name] = { patches: syncds, hasMorePatches, ...(snapshot ? { snapshot } : {}) }
		})
	)
...
export const chatModificationToAppPatch = (mod: ChatModification, jid: string) => {
	...
	const getMessageRange = (lastMessages: LastMessageList) => {
		...
		if (Array.isArray(lastMessages)) {
			...
			messageRange = {
				lastMessageTimestamp: lastMsg?.messageTimestamp ?? null,
				messages: lastMessages?.length
					? lastMessages.map(m => {
							...
							// coerce key fields to non-undefined values expected by types
							m.key.id = m.key.id ?? null
							m.key.remoteJid = m.key.remoteJid ?? null
							if (m.key.participant) {
								m.key.participant = jidNormalizedUser(m.key.participant)
							}
	...
	if ('mute' in mod) {
		patch = {
			syncAction: {
				muteAction: {
					muted: !!mod.mute,
					muteEndTimestamp: mod.mute ?? null
				}
			},
			index: ['mute', jid],
			type: 'regular_high',
		...
		patch = {
			syncAction: {
				labelEditAction: {
					name: mod.addLabel.name ?? null,
					color: mod.addLabel.color ?? null,
					predefinedId: mod.addLabel.predefinedId ?? null,
					deleted: mod.addLabel.deleted ?? false
				}
			},
...
export const processSyncAction = (
	...
	if (action?.muteAction) {
		...
		{
			id: id ?? null,
				...(typeof action.muteAction?.muted !== 'undefined' ? { muteEndTime: action.muteAction?.muted ? toNumber(action.muteAction.muteEndTimestamp) : null } : {}),
				...(isInitialSync ? { conditional: getChatUpdateConditional(id ?? '', undefined) } : {})
			}
		] as any)
		...
		ev.emit('chats.update', [
			{
				id: id ?? null,
				...(typeof isArchived !== 'undefined' ? { archived: isArchived } : {}),
				...(isInitialSync ? { conditional: getChatUpdateConditional(id ?? '', msgRange) } : {})
			}
		] as any)
		...
		ev.emit('chats.update', [
			{
				id: id ?? null,
				...(typeof (isNullUpdate ? null : !!markReadAction?.read ? 0 : -1) !== 'undefined'
					? { unreadCount: isNullUpdate ? null : !!markReadAction?.read ? 0 : -1 }
					: {}),
				...(isInitialSync ? { conditional: getChatUpdateConditional(id ?? '', markReadAction?.messageRange) } : {})
			}
		] as any)
		...
		ev.emit('contacts.upsert', [
			{
				...
				...(action.contactAction.lidJid ? { lid: action.contactAction.lidJid } : {}),
				...(action.contactAction.pnJid ? { phoneNumber: action.contactAction.pnJid } : {})
			}
		] as any)
		...
		ev.emit('messages.update', [
			{
				key: { remoteJid: id ?? null, id: msgId ?? null, fromMe: fromMe === '1' },
				update: { starred }
			}
		] as any)
	} else if (action?.deleteChatAction || type === 'deleteChat') {
		if (!isInitialSync) {
			ev.emit('chats.delete', [id!] as any)
		}
		...
		ev.emit('labels.edit', {
			...
			predefinedId: predefinedId ? String(predefinedId) : ''
		} as any)
