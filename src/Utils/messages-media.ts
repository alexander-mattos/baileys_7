...
export const getHttpStream = async (url: string | URL, options: RequestInit & { isStream?: true } = {}) => {
	const init = trimUndefined({ dispatcher: options.dispatcher, method: 'GET', headers: options.headers }) as unknown as RequestInit
...
export const downloadEncryptedContent = async (
	...
	// typing Crypto.Decipher is problematic across node/lib types; use any here
	let aes: any
		...
		for (const { hostname } of hosts) {
			...
			try {
				...
				const init = trimUndefined({
					...
					signal: timeoutMs ? AbortSignal.timeout(timeoutMs) : null
...
export const encryptMediaRetryRequest = async (key: WAMessageKey, mediaKey: Buffer | Uint8Array, meId: string) => {
	const recp: proto.IServerErrorReceipt = { stanzaId: key.id ?? null }
...
export const decodeMediaRetryNode = (node: BinaryNode) => {
	...
	const event: BaileysEventMap['messages.media-update'][number] = {
		key: {
			...
			remoteJid: rmrNode.attrs.jid ?? null,
			fromMe: rmrNode.attrs.from_me === 'true',
			participant: rmrNode.attrs.participant ?? null
		}
	}
